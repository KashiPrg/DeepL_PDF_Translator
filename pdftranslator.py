...
class PDFTranslator:
    def __init__(
        ...
        output_start_lines=False,
        output_end_lines=False,
        output_ignore_lines=False,
        output_replace_source_lines=False,
        output_markdown_replace_source_lines=False,
        output_return_lines=False,
        output_return_ignore_lines=False,
        output_header_lines=False
    ):
        """
        Args:
            ...
            source_as_comment (bool): Markdown形式において、原文をコメントとして出力するか
            ignore_start_condition (bool): 翻訳開始条件を無いものとして扱うか
            ignore_end_condition (bool): 翻訳終了条件を無いものとして扱うか
            output_start_lines (bool, optional): 翻訳開始条件に該当する行をtxtファイルとして出力するか
            output_end_lines (bool, optional): 翻訳終了条件 〃
            output_ignore_lines (bool, optional): 無視条件 〃
            output_replace_source_lines (bool, optional): 置換条件 〃
            output_markdown_replace_source_lines (bool, optional): 置換条件(Markdown出力時) 〃
            output_return_lines (bool, optional): 改行条件 〃
            output_return_ignore_lines (bool, optional): 改行無視条件 〃
        ...
        self.__source_as_comment = source_as_comment
        self.__ignore_start_condition = ignore_start_condition
        self.__ignore_end_condition = ignore_end_condition
        self.__output_start_lines = output_start_lines
        self.__output_end_lines = output_end_lines
        self.__output_ignore_lines = output_ignore_lines
        self.__output_replace_source_lines = output_replace_source_lines
        self.__output_markdown_replace_source_lines = output_markdown_replace_source_lines
        self.__output_return_lines = output_return_lines
        self.__output_return_ignore_lines = output_return_ignore_lines
        self.__output_header_lines = output_header_lines

    def PDFTranslate(self, filename):
        textlines = self.PDFTextExtract(filename)
        ...
        if textlines is None:
            ...
            while True:
                ...
                if not self.__ignore_start_condition:
                ...
                if not self.__ignore_end_condition:
                ...
                if ignore_start_condition or ignore_end_condition:
                    ...
                    textlines = self.PDFTextExtract(filename, ignore_start_condition, ignore_end_condition)
                else:
        ...
        tl_units = self.OrganizeTranslationUnits(filename, textlines)

        # 翻訳の実行と翻訳結果の書き込み
        self.TranslateAndWrite(filename, tl_units)
    ...
    def PDFTextExtract(
        self,
        filename,
        force_ignore_start_condition=False,
        force_ignore_end_condition=False
    ):
        """
        ...
        # 開始条件を無視する場合
        if self.__ignore_start_condition or force_ignore_start_condition:
        ...
        for t in temp:
            ...
            for sl in RegularExpressions.start_lines:
                if re.search(sl, t, flags=re.IGNORECASE):
                    if self.__output_start_lines:
                        f_start.write(sl + ", " + t + "\n")
                    if not (self.__ignore_start_condition or force_ignore_start_condition):
                        lines_extracting = True
                    break
            # 翻訳を打ち切る合図となる文字列を探す
            for el in RegularExpressions.end_lines:
                if re.search(el, t, flags=re.IGNORECASE):
                    if self.__output_end_lines:
                        f_end.write(el + ", " + t + "\n")
                    if not (self.__ignore_end_condition or force_ignore_end_condition):
            ...
            for ri in range(len(RegularExpressions.replace_source)):
                if self.__output_replace_source_lines and re.search(RegularExpressions.replace_source[ri], t):
            ...
            for mri in range(len(RegularExpressions.markdown_replace_source)):
                if self.__output_markdown_replace_source_lines and re.search(RegularExpressions.markdown_replace_source[mri], t):
    ...
    def OrganizeTranslationUnits(self, filename, textlines, tl_unit_max_len=4800):
        """
        ...
        if self.__output_return_lines:
            f_return = open(file_path + "_Return.txt", mode="w", encoding="utf-8")
        if self.__output_return_ignore_lines:
    ...
    def TranslateAndWrite(self, filename, tl_units):
        # 出力用のディレクトリを作成
        Path("output").mkdir(exist_ok=True)
        # 出力用ファイルのパス
