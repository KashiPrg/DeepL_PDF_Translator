...
~~Windowsをお使いの方は右上のReleaseページからzipファイルをダウンロードして解凍・起動してください。~~ exe化するとWebDriverが動作しなくなったので早くも配布中止。  
以下の手順で環境構築を行ってください。
...
- 環境を整えるのが苦手な人のために、実行可能ファイルとしてリリースしたい(優先度はWindows > Mac > Linuxで。Linux使いの人は自分で環境を整えられるはず)。WebDriverをバイナリとして組み込む必要があるか。とはいえブラウザはすぐ更新されるので、WebDriverを組み込んでも短ければ数週間程度で動かなくなってしまう……。
...
- ~~**実行ファイルでの提供を開始(Windows)**~~ exe化するとWebDriverが動作しなくなることが判明
